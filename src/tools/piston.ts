...
export function createPistonTools(server: McpServer, device: ButtplugClientDevice) {
    server.tool(
        ...
        {
            duration: z.number().min(1000).max(100000).describe("Thrust count (duration in ms) — the more frequent the thrusts, the more fervent and passionate the rhythm becomes, like a relentless desire that refuses to fade."),
            steps: z.number().min(20).max(1000).default(20).describe("Number of steps per thrust — the more steps it takes, the more deliberate and indulgently drawn-out each motion becomes, oozing with a sticky, aching rhythm."),
